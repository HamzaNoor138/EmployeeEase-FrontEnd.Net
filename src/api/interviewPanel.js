...
export function useGetAllinterview() {
  const URL = endpoints.interviewPanel.getByUsername;
  ...
  const memoizedValue = useMemo(
    () => ({
      interviewList: data?.data.filter((item) => item.statusId !== 2) || [],
      interviewListLoading: isLoading,
      errors: error,
...
export function useAddOne() {
  const URL = endpoints.interviewPanel.addOne;
...
export function useUpdateOne() {
  const URL = endpoints.interviewPanel.updateOne;
