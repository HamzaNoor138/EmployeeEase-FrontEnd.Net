...
export function useGetAllProfessions() {
  const URL = endpoints.profession.getAll;
  ...
  const memoizedValue = useMemo(
    () => ({
      professionList: data?.data.filter((item) => item.statusId !== 2) || [],
      professionLoading: isLoading,
      errors: error,
...
export function useAddOne() {
  const URL = endpoints.profession.addOne;
...
export function useUpdateOne() {
  const URL = endpoints.profession.updateOne;
