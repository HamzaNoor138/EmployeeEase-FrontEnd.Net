...
export function useGetAllDesignations() {
  const URL = endpoints.designation.getAll;
  ...
  const memoizedValue = useMemo(
    () => ({
      designationList: data?.data.filter((item) => item.statusId !== 2) || [],
...
export function useAddOne() {
  const URL = endpoints.designation.addOne;
...
export function useUpdateOne() {
  const URL = endpoints.designation.updateOne;
