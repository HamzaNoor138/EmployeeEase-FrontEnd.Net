...
export function useGetAllState() {
  const URL = endpoints.addState.getAll;
  ...
  const memoizedValue = useMemo(
    () => ({
      stateList: data?.data.filter((item) => item.statusId !== 2) || [],
...
export function useAddOne() {
  const URL = endpoints.addState.addOne;
...
export function useUpdateOne() {
  const URL = endpoints.addState.updateOne;
