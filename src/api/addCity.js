...
export function useGetAllCity() {
  const URL = endpoints.addCity.getAll;
  ...
  const memoizedValue = useMemo(
    () => ({
      cityList: data?.data.filter((item) => item.statusId !== 2) || [],
...
export function useAddOne() {
  const URL = endpoints.addCity.addOne;
...
export function useUpdateOne() {
  const URL = endpoints.addCity.updateOne;
