...
import {
  useAddOne,
  useUpdateOne as EmployeeAccountUpdate,
  useGetEmployeeAccountInfo,
} from 'src/api/employeePaymentInfo';
...
export default function ProfileHome({
  ...
  const { submitFormEmployeePayment } = useAddOne();
  const { employeePaymentInfoUpdate } = EmployeeAccountUpdate();
  const { EmployeeaccountInfoList, refetchData: refetchEmployeeAccountData } =
    useGetEmployeeAccountInfo();
  ...
  useEffect(() => {
    if (currentTab == 'profile' || currentTab == 'EditProfile') {
      if (
        ...
        const mappedSkils = skillList.map((skill) => ({
          key: skill.skillId,
          value: skill.fullName,
        }));

        setDummyMultiselectObj(mappedSkils);

        setMultiSelectValues(CandidateList[0].skills, mappedSkils, setDummyDefaultValue);
        CandidateList[0].experience = CandidateList[0].experience.trim();
      ...
      if (
        EmployeeaccountInfoList &&
        EmployeeaccountInfoList.length > 0 &&
        EmployeeaccountInfoList[0] != null
      ) {
        setSelectedRow2(EmployeeaccountInfoList[0]);
        setShowbutton(true);
        setIsAddMode(false);
        setErrors('');
      } else {
    ...
    if (currentTab == 'profile' || currentTab == 'EditProfile') {
      const Fields = [
...

        {
          ...
          disabled: disableAllFields,
          options: [
            { value: 'Fresh Graduate', key: 'freshGraduate' },
            ...Array(11)
              .fill()
              .map((_, i) => ({
                value: `${i} year${i > 1 ? 's' : ''}`,
  ...
  const handleFormSubmit = async (data) => {
    if (currentTab == 'EditProfile') {
      const updatedData = {
        ...
        averagePerformanceScore: data?.averagePerformanceScore || 0,
        companyJoinedDate: data?.companyJoinedDate || null,
        ...
        if (isAddMode === true) {
          ...
          try {
            ...
            if (response && response.code === '500' && response.message) {
              ...
              await refetchData();
            } else if (response && response.code === '409' && response.message) {
              if (response.message == 'phoneNumber') {
                setErrors('Phone number Already exist');
              } else if (response.message == 'cnic') {
                setErrors('CNIC Already exist');
              }
            } else {
              quickEdit.onFalse();
      ...
      const updatedData = {
        candidateId: data?.candidateId,
        employeePaymentId: data?.employeePaymentId,
        employeeusername: sessionStorage.getItem('username'),
      ...
      if (!newErrorDetails) {
        if (isAddMode === true) {
          await submitFormEmployeePayment(updatedData);
          quickEdit.onFalse();
          enqueueSnackbar('New Record successfully added');
          setSelectedRow(null);
          await refetchEmployeeAccountData();
        } else {
          try {
            const response = await employeePaymentInfoUpdate(updatedData);
            if (response && response.code === '500' && response.message) {
              enqueueSnackbar(response.message, {
                variant: 'border',
                style: { backgroundColor: '#FF5630', color: '#fff' },
