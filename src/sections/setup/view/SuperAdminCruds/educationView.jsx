...
import { useGetAlleducation, useAddOne, useUpdateOne } from 'src/api/education';
...
export default function EmployeeView() {
  ...
  const { educationList, refetchData, educationLoading } = useGetAlleducation();
  ...
  }, [educationList, tableData]);

  let addTitle1 = '';
  const handleAddClick = async () => {
    addTitle1 = 'Add Education Information';
    setErrors('');
    setTitle(addTitle1);
  ...
  const handleEditClick = async (row) => {
    setErrors('');
    addTitle1 = 'Update Education Information';
    setTitle(addTitle1);
    setSelectedRow(row);
    setIsAddMode(false);
    setSpinner(true);
  ...
  const handleFormSubmit = async (data) => {
    ...
    const updatedData = {
      ...
      code: data?.code.trim() || '',
      statusId: data?.statusId || 0,
      yearsOfStudy: data?.yearsOfStudy || 0,
    };
    // setSelectedRow(updatedData);
    let newErrorDetails = '';
    ...
    if (
      educationList.some(
        (item) =>
          ...
          item.educationId !== data.educationId
      )
    ) {
      newErrorDetails = 'Education Already Exist';
    } else if (
      educationList.some(
        (item) =>
      ...
      newErrorDetails = 'Education Code Already Exist';
    }
  ...
  const gridData = educationList.map((country, index) => ({
    id: index + 1,
    educationId: country.educationId,
    code: country.code,
    fullName: country.fullName.trim(),
    dated: country.dated.trim(),
    statusId: country.statusId,

    yearsOfStudy: country.yearsOfStudy,
  }));
  return (
    <div className={`main-container ${loadingSpinner ? 'blur' : ''}`}>
      {loadingSpinner && <Spinner />}
      <Helmet>Education</Helmet>
      <CustomBreadcrumbs
        heading="Education"
        links={[{ name: 'Dashboard' }, { name: 'Education' }, { name: 'List' }]}
        action={
