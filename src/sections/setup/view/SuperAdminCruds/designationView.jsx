...
import { useGetAllDesignations, useAddOne, useUpdateOne } from 'src/api/designation';
...
const TABLE_HEAD = [
  ...
  { field: 'fullName', headerName: 'DesignationName', width: 210, flex: 1 },
  { field: 'code', headerName: 'DesignationCode', width: 150, flex: 1 },
  {
    field: 'statusId',
    headerName: 'Status',
    width: 100,
...
export default function EmployeeView() {
  ...
  const { designationList, refetchData, countryLoading } = useGetAllDesignations();
  ...
  useEffect(() => {
    if (designationList.length) {
      setTableData(designationList);
    }
  }, [designationList, tableData]);

  const handleAddClick = async () => {
    setTitle('Add Designation Information');
    setErrors('');
    setSelectedRow(null);
    setIsAddMode(true);
    quickEdit.onTrue();
  ...
  const handleEditClick = async (row) => {
    setErrors('');
    setTitle('Update Designation Information');
    setSelectedRow(row);
    setIsAddMode(false);
    setSpinner(true);
    setSpinner(false);
  ...
  const handleFormSubmit = async (data) => {
    ...
    if (
      ...
      newErrorDetails = 'Designation Already Exist';
    } else if (
      designationList.some(
        (item) =>
      ...
      newErrorDetails = 'Designation Code Already Exist';
    }
  ...
  const gridData = designationList.map((country, index) => ({
    id: index + 1,
    designationId: country.designationId,
    code: country.code,
  ...
  return (
    <div className={`main-container ${loadingSpinner ? 'blur' : ''}`}>
      ...
      <Helmet>Designation</Helmet>
      <CustomBreadcrumbs
        heading="Designation"
        links={[{ name: 'Dashboard' }, { name: 'Designation' }, { name: 'List' }]}
        action={
          <Button
