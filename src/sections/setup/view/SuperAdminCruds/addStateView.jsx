...
import { useGetAllState, useAddOne, useUpdateOne } from 'src/api/addState';
...
const TABLE_HEAD = [
  ...
  { field: 'fullName', headerName: 'State Name', width: 210, flex: 1 },
  { field: 'code', headerName: 'State Code', width: 150, flex: 1 },
  { field: 'countryName', headerName: 'Country', width: 200, flex: 1 }, // New column for country name
  {
    field: 'statusId',
    headerName: 'Status',
    width: 100,
...
export default function AddStateView() {
  ...
  //     }));
  //     setDummyMultiselectObj(mappedCities);
  //   }
  // }, [cityList]);
  const fields = [
    {
  ...
  const handleEditClick = async (row) => {
    ...
    setSpinner(true);
    setSpinner(false);
    // setMultiSelectValues(row.cityIds, dummyMultiselectObj, setDummyDefaultValue);
    quickEdit.onTrue();
  };
  ...
  const handleFormSubmit = async (data) => {
    ...
    if (
      ...
      newErrorDetails = 'State Already Exists';
    } else if (
      stateList.some(
        (item) =>
      ...
      newErrorDetails = 'State Code Already Exists';
    }
  ...
  const gridData = stateList.map((state, index) => ({
    id: index + 1,
    ...
    countryName:
      countryList.find((country) => country.countryId === state.countryId)?.fullName || '', // Fetch country name based on countryId
  }));

  return (
    <div className={`main-container ${loadingSpinner ? 'blur' : ''}`}>
      {loadingSpinner && <Spinner />}
      <Helmet>AddState</Helmet>
      <CustomBreadcrumbs
        heading="AddState"
        links={[{ name: 'Dashboard' }, { name: 'AddState' }, { name: 'List' }]}
        action={
          <Button
